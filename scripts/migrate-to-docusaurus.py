...
class DocusaurusMigrator:
    ...
    def migrate_frontmatter(self, content: str) -> str:
        ...
        # Reconstruct
        if docusaurus_fm:
            new_frontmatter = yaml.dump(docusaurus_fm, default_flow_style=False, sort_keys=False)
            return f"---\n{new_frontmatter}---\n{body}"
        else:
            return body
...
Examples:
  ...
  python migrate-to-docusaurus.py --source ../. --target ./docs --verbose
    ...
    if target_path.exists() and list(target_path.glob('*.md')):
        response = input(f"Target directory {target_path} contains markdown files. Overwrite? [y/N]: ")
        if response.lower() != 'y':
