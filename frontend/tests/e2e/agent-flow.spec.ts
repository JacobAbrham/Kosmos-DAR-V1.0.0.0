...
test.describe('Agent Interaction Flow', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await expect(page.locator('input[placeholder*="Message KOSMOS"]')).toBeVisible({ timeout: 5000 });
    });

    test('should show system initialization message', async ({ page }) => {
        await expect(page.locator('input[placeholder*="Message KOSMOS"]')).toBeVisible();
    });

    test('should receive AI-generated response to user query', async ({ page }) => {
        const chatInput = page.locator('input[placeholder*="Message KOSMOS"]').first();
